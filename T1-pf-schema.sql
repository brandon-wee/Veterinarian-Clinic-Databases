...
CREATE TABLE visit (
    ...
    visit_notes VARCHAR2(200) NOT NULL,
    visit_weight NUMBER(4, 1) NOT NULL,
    visit_total_cost NUMBER(6, 2) NOT NULL,
    animal_id NUMBER(5),
    vet_id NUMBER(4),
    clinic_id NUMBER(2),
...
COMMENT ON COLUMN visit.visit_id IS 'Identifies for visit (added surrogate key)';
COMMENT ON COLUMN visit.visit_date_time IS 'Date and time of visit';
COMMENT ON COLUMN visit.visit_length IS 'Visit length in minutes (must be between 30-90 minutes, inclusive)';
...
COMMENT ON COLUMN visit.from_visit_id IS 'The previous visits identifier (for follow-up visit only)';
...
COMMENT ON COLUMN visit_drug.visit_id IS 'Identifier for visit (added surrogate key)';
...
COMMENT ON COLUMN visit_service.visit_id IS 'Identifier for visit (added surrogate key)';
...
-- VISIT
ALTER TABLE visit
    ADD CONSTRAINT animal_id_fk FOREIGN KEY ( animal_id )
        REFERENCES animal ( animal_id );

ALTER TABLE visit
    ADD CONSTRAINT vet_id_fk FOREIGN KEY ( vet_id )
        REFERENCES vet ( vet_id );

ALTER TABLE visit
    ADD CONSTRAINT clinic_id_fk FOREIGN KEY ( clinic_id )
        REFERENCES clinic ( clinic_id );

ALTER TABLE visit
    ADD CONSTRAINT from_visit_id_fk FOREIGN KEY ( from_visit_id )
        REFERENCES visit ( visit_id );


-- VISIT_DRUG
ALTER TABLE visit_drug 
    ADD CONSTRAINT visit_id_fk FOREIGN KEY ( visit_id )
        REFERENCES visit ( visit_id );

ALTER TABLE visit_drug
    ADD CONSTRAINT drug_id_fk FOREIGN KEY ( drug_id )
        REFERENCES drug ( drug_id );


-- VISIT_SERVICE
ALTER TABLE visit_service
    ADD CONSTRAINT visit_service_id_fk FOREIGN KEY ( visit_id )
        REFERENCES visit ( visit_id );

ALTER TABLE visit_service
    ADD CONSTRAINT service_code_fk FOREIGN KEY ( service_code )
